...
const navigationItems = [
  { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
  {
    ...
    icon: Tag,
    subItems: [
      { name: 'Products', href: '/products/lists' },
      { name: 'Categories', href: '/products/categories' },
      { name: 'Suppliers', href: '/products/suppliers' },
    ],
  },
  { name: 'Orders', href: '/orders/orders', icon: ListOrdered },
  ...
  // { name: 'Employees', href: '/employees', icon: UserCheck },
  {
    ...
    subItems: [
      ...
      { name: 'Escrow', href: '/reports/escrow' },
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { user, logout } = useAuthLayout()
        ...
        <div className="flex items-center h-16 px-4 border-b">
          ...
          {sidebarOpen ? (
            <a href="/" onClick={(e) => { e.preventDefault(); handleNavigation('/'); }} className="flex items-center space-x-2">
              <Image src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/logo-lzkk0K2zfwnPemdMiB5zEOX2vnKl5Q.png" alt="SalesBox Logo" width={150} height={150} />
            </a>
          ) : (
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <Avatar className="h-8 w-8">
                <AvatarImage src="https://cdn-icons-png.flaticon.com/128/1999/1999625.png" alt={user?.username || "User"} />
                <AvatarFallback>{user?.username ? user.username[0].toUpperCase() : "U"}</AvatarFallback>
              </Avatar>
              <span className="text-sm font-medium">{user?.username || "User"}</span>
              <LogOut className="h-5 w-5 ml-2 cursor-pointer text-red-500" onClick={handleLogout} />
            </div>
          </div>
        </header>
